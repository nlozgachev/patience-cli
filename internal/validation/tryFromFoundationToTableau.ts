...
export function tryFromFoundationToTableau(ctx: MoveData): Operation<FromFoundationToTableau> {
  if (ctx.from.pile !== FOUNDATION || ctx.to.pile !== TABLEAU) return err(WRONG_VALIDATOR);
