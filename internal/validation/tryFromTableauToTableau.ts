...
export function tryFromTableauToTableau(ctx: MoveData): Operation<FromTableauToTableau> {
  if (ctx.from.pile !== TABLEAU || ctx.to.pile !== TABLEAU) return err(WRONG_VALIDATOR);
